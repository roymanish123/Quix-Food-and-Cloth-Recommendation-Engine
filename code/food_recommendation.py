...
=======================
Food Recommendation Engine for Quix Recommendation Engine project.

Two modes:
  Mode 1 — Quick: location + month → 10 random images from ALL meal types combined
  Mode 2 — Full:  location + month + meal preference + time of day → full folder + metadata
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
BASE_DIR = r"dataset\food\images"

# Season map per location per month (matches food folder naming)
SEASON_MAP = {
    "dubai": {
...
VALID_MEAL_PREFS   = ["veg", "vegan", "egg", "non_veg"]
    ...
    while True:
        ...
        if parser:
            ...
            if result is not None:
                return result
            print(f"  ❌ Invalid. Valid options: {', '.join(str(o) for o in valid_options)}\n")
        else:
            if raw in valid_options:
                return raw
            print(f"  ❌ Invalid. Valid options: {', '.join(valid_options)}\n")
...
def open_images(image_paths):
    """Open images using Windows default viewer."""
...
def display_metadata(folder_path, image_files):
    """Display folder context + per-image metadata from metadata.json."""
    ...
    print(f"  Meal Preference : {ctx.get('meal_preference', 'N/A')}")
    ...
    print(f"\n  🍽️  FOOD ITEMS ({len(image_files)} images)\n")

    for i, img_file in enumerate(image_files, 1):
        item = item_lookup.get(img_file, {})
        print(f"  [{i:02d}] {img_file}")
        print(f"       Dish Name       : {item.get('dish_name', item.get('item_name', 'N/A'))}")
        print(f"       Category        : {item.get('category', 'N/A')}")
        print(f"       Meal Preference : {item.get('meal_preference', 'N/A')}")
        print(f"       Time of Day     : {item.get('time_of_day', 'N/A')}")
...
def mode_1():
    divider()
    print("  🍽️  MODE 1 — QUICK RECOMMENDATION")
    print("  Input: Location + Month")
    print("  Output: 10 random food images from ALL meal types")
    divider()
    ...
    print(f"\n  🔍 Scanning all meal types for {location} / {month_folder} / {season}...")

    # Collect ALL images across all meal types and all time slots for this month+season
    all_images = []   # list of absolute paths

    for meal_pref in VALID_MEAL_PREFS:
        for time_slot in VALID_TIMES:
            folder_path = os.path.join(
                BASE_DIR, location, meal_pref, month_folder, season, time_slot
            )
    ...
    divider("-")
    print(f"\n  {'#':<5} {'Filename':<15} {'Meal Type':<12} {'Time Slot'}")
    print(f"  {'-'*4:<5} {'-'*14:<15} {'-'*11:<12} {'-'*9}")
    for i, img in enumerate(picked, 1):
        print(f"  {i:<5} {img['filename']:<15} {img['meal_pref']:<12} {img['time_slot']}")

    print()
    divider("-")
    print(f"  🖼️  Opening {count} random food images...")
    divider("-")
...
def mode_2():
    divider()
    print("  🍽️  MODE 2 — FULL RECOMMENDATION")
    print("  Input: Location + Month + Meal Preference + Time of Day")
    ...
    # Meal Preference
    print(f"\n  Meal preferences: {', '.join(VALID_MEAL_PREFS)}")
    meal_pref = get_input("  Enter meal preference: ", VALID_MEAL_PREFS)

    # Time of Day
    print(f"\n  Time of day: {', '.join(VALID_TIMES)}")
    time_of_day = get_input("  Enter time of day: ", VALID_TIMES)

    # Build path
    folder_path = os.path.join(
        BASE_DIR, location, meal_pref, month_folder, season, time_of_day
    )
...
def run_engine():
    divider()
    print("       🍽️   QUIX FOOD RECOMMENDATION ENGINE")
    divider()
    ...
    while True:
        ...
        print("  2 → Full   (location + month + meal preference + time of day)")
        divider("-")
